  ...
  <div>
    ...
    <div class="box">
      <div class="box-body">
        ...
        <div class="row">
            <div class="col-md-4">
                <router-link class="btn btn-default" :to="'/talent/team/reservation/past'"><i class="fa fa-backward"></i> Past Reservation</router-link>
            </div> 
        ...
        <table class="table" v-if="data.total > 0">
          ...
          <tbody>
            <tr v-for="data in data.list">
              ...
              <!--<td><a @click="cancelReservation(data.workshop.id, data.member.id)" class="btn btn-danger btn-sm"><i class="fa fa-calendar-minus-o"></i> Cancel</a></td>-->
            </tr>
          </tbody>
        </table>
        <div v-else>
  ...
  export default {
    ...
    methods: {
      getData: function () {   
        ...
        auth.getData2(this, '/talent/workshop_reservation/upcoming?page='+ this.page.page +'&page_size='+ this.page.size + '&' + status1)
                .then(res => {
                  if(res.status === 204){
                    console.log(res)
      ...
      cancelReservation: function(id, memberId){
        this.loading = true
        auth.putData2(this, '/talent/member/'+memberId+'/workshop_reservation/'+ id)
                .then(res => {
                  console.log(res)
                  this.loading = false
                  this.getData()
